...
\begin{block}{Neutrino oscillation with wave packets}
  \begin{itemize}
      ...
      \item Assuming formfactor is Gaussian the oscillation probability reads: 
        \begin{equation*}
        \hspace*{-2cm} P_{\alpha\beta}(L) = \sum_{k,\,j=1}^3\frac{ V^{\phantom\dagger}_{k \beta }V^*_{\alpha k}V^{\phantom\dagger}_{j \alpha }  V^*_{\beta j} }{\sqrt[4]{1 +
    \left(L/L^{\text{d}}_{kj}\right)^2}}\,\,
    \exp{\left[- \frac{\left(L/\Lcoh_{kj}\right)^2}{1+\left(L/\Ld}_{kj}\right)^2} -\mathrm{D}^2_{kj}\right]}
    ...
    production and detection proccesses.\\ In that poster 
    ...
    \begin{columns}[T]
        \begin{column}{0.58\textwidth}
            \begin{itemize}
                ...
                \item \Important{6} nuclear reactors with 2.9 GW of thermal power  
                \item \anue\, detection through IBD 
                    {\centering{\Important{$\anue + p \rightarrow e^+ + n$}}}
